...
@php
$page_title =  Session::get('cms_name') . "| My Reservation";
@endphp
      ...
      <div class="container shopping-cart">
        <div class="row mt-5">
            <div class="col-md-12">
              <div class="card">
                <div class="card-body">
                    <h3>My Reservations ({{ isset($orders) ? count($orders) : "" }} items)</h3>
                    @php
                        $order_number_temp = "";
                        $total = 0;
                    @endphp
                    @foreach ($orders as $key => $item)
                        ...
                        @if ($item->status == 2 || $item->status == 3)
                          <div class="float-right mt-3">Estimated Ready date <br> <span>{{date('F d, Y', strtotime($item->delivery_date))}}</span></div>
                        @endif
                        ...
                        <div>Date order: {{date('F d, Y h:i A', strtotime($item->created_at))}} </div>
                        <a href="/preview-order-ereceipt/{{ $item->order_no }}" class="btn btn-info" role="button" target="_blank">View Reservation Receipt</a><br>
                          @if ($item->status != 0) 
