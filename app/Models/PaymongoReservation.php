...
class PaymongoReservation extends Model
{
    ...
    public function createPaymayaPaymentIntent2($amount) {
        $client = new \GuzzleHttp\Client();
    ...
    public function attatchPaymayaPaymentIntent2($payment_intent_id, $payment_method_id) {
        $client = new \GuzzleHttp\Client();
            ...
            'body' => '{
              "data":{
                "attributes":
                {
                  ...
                  "return_url":"'.route('createPayment2').'"
                }
              }
            }',
            'headers' => [
    ...
    public function createSource2($amount) {
        $client = new \GuzzleHttp\Client();
            ...
            'body' => '{
                "data":
                {
                    "attributes":
                    {
                        "amount":'.$amount.',
                        "redirect":{
                            "success":"'.route('createPayment2').'",
                            "failed":"'.route('createPayment2').'"
                        },
                        "type":"gcash",
                        "currency":"PHP"
                    }
    ...
    public function createPayment() {
            ...
            && $source->data->attributes->status == 'chargeable') 
        {
            ...
            if ($payment_info->data->attributes->status == 'paid') {
                ...
                return redirect( env('APP_URL').'/place-order-gcash-re');
            }
            else {
          ...
          return redirect(env('APP_URL').'/place-order-paymaya-re');
        }
        else {
          return response()->json([
              'status' =>  'error',
          ...
          'body' => '{
            "data": {
              "attributes": {
                ...
                "description": "Reservation From '.$product_name->name.'",
                "line_items": [
                  {
                    "currency": "PHP",
                    "amount": '.$amount.',
                ...
                "success_url": "http://localhost:8000/place-order-card-re"
              }
            }
          }',
          'headers' => [
