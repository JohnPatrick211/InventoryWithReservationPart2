...
function getItems(data) {
    ...
    if (data.qty == 0) {
        ...
        html += '</div>';
        html += '<div class="d-flex">';
        html += '<input class="qty-'+data.id+'" onkeydown="return (event.keyCode!=189);" type="number" min="1" value="1" style="width:50px; margin-left:5px;">';
        html += '<button class="btn btn-sm btn-success ml-2 btn-add-to-pre-order" data-id="'+data.id+'" data-product-code="'+data.product_code+'">Pre-order</button>';
    }else {
        html += '<div class="card-text"> Stock: <span class="stock-'+data.id+'">'+data.qty+'</span></div>';
        html += '</div>';
        html += '<div class="d-flex">';
        html += '<span>Qty</span><input class="qty-'+data.id+'" onkeydown="return (event.keyCode!=189);" type="number" min="1" value="1" style="width:50px; margin-left:5px;">';
        html += '<button class="btn btn-sm btn-success ml-2 btn-add-to-cart" data-id="'+data.id+'" data-product-code="'+data.product_code+'">Add to Cart</button>';
        html += '<button class="btn btn-sm btn-success ml-2 btn-add-to-reservation" data-id="'+data.id+'" data-product-code="'+data.product_code+'">Add to Reservation</button>';
    }
    html += '</div>';
    html +=' </div>';
    html += '</div>';
...
async function onClick () {
    ...
    $(document).on('click', '.btn-add-to-cart', async function(){
        ...
        $.ajax({
            ...
            success:async function(data){
                ...
                if (data.status == 'not_auth') {
                ...
                else if (data.status == 'not_auth_qty') {
                    ...
                    $this.html('Add to Cart');
                    $.toast({
                        heading:'The Product Qty is Greater Than Stock',
                        showHideTransition: 'plain',
                        position: 'bottom-right',
                        hideAfter: 4000, 
                    });
    ...
    $(document).on('click', '.btn-add-to-reservation', async function(){
        var $this = $(this);
        var product_code = $(this).attr('data-product-code');
        var id           = $(this).attr('data-id');
        var qty          = $('.qty-'+id).val();
        ...
        $.ajax({
            ...
            success:async function(data){
                ...
                reservationCount();
                    $.toast({
                        heading:'Item was successfully added to Reservation! ',
                        showHideTransition: 'plain',
                        position: 'bottom-right',
                        hideAfter: 4000, 
                    });
    ...
    $(document).on('click', '.btn-add-to-pre-order', async function(){
        var $this = $(this);
        var product_code = $(this).attr('data-product-code');
        var id           = $(this).attr('data-id');
        var qty          = $('.qty-'+id).val();
